...
class HomeController extends GetxController {
  ...
  _initData() async {
    ...
    // 保存离线数据
    Storage().setJson(Constants.storageHomeBanner, await SystemApi.banners());
    Storage().setJson(
      Constants.storageHomeCategories,
      await ProductApi.categories(),
    );
    Storage().setJson(
      Constants.storageHomeFlashSell,
      await ProductApi.products(ProductsReq(featured: true)),
    );
    Storage().setJson(
      Constants.storageHomeNewSell,
      await ProductApi.products(ProductsReq()),
    );
    // 保存离线数据 - 基础数据
    Storage().setJson(
