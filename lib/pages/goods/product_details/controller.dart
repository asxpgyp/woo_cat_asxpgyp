...
class ProductDetailsController extends GetxController
  ...
  _loadProduct() async {
    ...
    if (product?.images != null) {
      bannerItems = product!.images!
          .map<KeyValueModel>(
            (item) => KeyValueModel(key: " ${item.id}", value: item.src ?? ""),
          )
          .toList();
    }
    // 选中值
    if (product?.attributes != null) {
      // 颜色
      var colorAttr = product?.attributes
      ...
      // 尺寸
      var sizeAttr = product?.attributes
          ?.where((e) => e.name == "Size")
          .toList();
    ...
    // 评论
    // reviews = await ProductApi.reviews(ReviewsReq(product: 13));
    // 评论图片，测试用
    reviewImages.addAll([
  ...
  // 星级选中
  void onStarTap(int keys, int index) {
    reviews[index].rating = keys;
